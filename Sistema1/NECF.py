Hello server!SW="Replicate_V1.0099999999999998"
HW="1.000"
...
#**INTENSIDAD.
class Intensidad:
    Lista_Capacidades_Necesaria=['ldr']
    def nivelluz(self):
...
#**DTH.
class Dth:
    Lista_Capacidades_Necesaria=['dth']
    def Temp(self):
        x=random.randint(10,40)
        return x
    def Hum(self):
...
#**MedirTemperatura.
    ...
    def MedirTemperatura():
        Lista_Capacidades_Necesaria=['dth']
        dth1=Dth()
        Temperatura_Medir=dth1.Temp()
        while True:
            print("Temperatura:"+str(Temperatura_Medir))
            Temperatura_Medir=dth1.Temp()
            time.sleep(Hilos_FP.tempo_MT)
#**end
#**MedirHumedad.
    tempo_HU=5
    def MedirHumedad():
        Lista_Capacidades_Necesaria=['dth']
        dth2=Dth()
        Humedad_Medir=dth2.Hum()
        while True:
            print("Humedad:"+str(Humedad_Medir))
            Humedad_Medir=dth2.Hum()
            time.sleep(Hilos_FP.tempo_HU)
#**end            
#**Bombadeagua.
    tempo_WP=5
    def UsarBombaDeAgua():
...
#**IntensidadLuz.
    ...
    def Nutrientes():
        Lista_Capacidades_Necesaria=['ldr']
        ldr=Intensidad()
        intendidadluz=ldr.nivelluz()
        while True:
            print("El nivel de luz es:"+str(intendidadluz))
            intendidadluz=ldr.nivelluz()
            time.sleep(Hilos_FP.tempo_NP)
#**end
#**Ph.
    tempo_PH=5
...
#""main
time.sleep(1)
HiloMedirTemperatura = threading.Thread(target=Hilos_FP.MedirTemperatura, args=(), daemon=False)
HiloMedirTemperatura.start()

HiloMedirHumedad = threading.Thread(target=Hilos_FP.MedirHumedad, args=(), daemon=False)
HiloMedirHumedad.start()

HiloUsarBombaDeAgua = threading.Thread(target=Hilos_FP.UsarBombaDeAgua, args=(), daemon=False)
HiloUsarBombaDeAgua.start()

HiloLeerNivelDeAgua = threading.Thread(target=Hilos_FP.LeerNivelDeAgua, args=(), daemon=False)
HiloLeerNivelDeAgua.start()

HiloNutrientes = threading.Thread(target=Hilos_FP.Nutrientes, args=(), daemon=False)
HiloNutrientes.start()

HiloControldeNiveldePh = threading.Thread(target=Hilos_FP.ControldeNiveldePh, args=(), daemon=False)
HiloControldeNiveldePh.start()

HiloControldeNiveldeNutrientes = threading.Thread(target=Hilos_FP.ControldeNiveldeNutrientes, args=(), daemon=False)
HiloControldeNiveldeNutrientes.start()

HiloLeerintensidadLuz = threading.Thread(target=Hilos_FP.LeerintensidadLuz, args=(), daemon=False)
HiloLeerintensidadLuz.start()

HiloEscuchaGeneral = threading.Thread(target=Hilos_Sistema.EscuchaGeneral, args=(), daemon=False)
HiloEscuchaGeneral.start()

HiloInicial = threading.Thread(target=Hilos_Sistema.Inicial, args=(), daemon=False)
HiloInicial.start()
#""end
"""
...
#**FANS.
class Fans:
    Lista_Capacidades_Necesaria=['fans']
    def fans(self, temperaturaactual):#Ventiladores
        if(temperaturaactual>36):
            print("Ventiladores encendidos 3 segundos")
#**end
#""end
...
#**ControldeVentiladores.
    tempo_CV=5
    def Ventiladores():
        Lista_Capacidades_Necesaria=['dth','fans']
        dth2=Dth()
        ventilador=Fans()
        Temp_Ventilador=dth2.Temp()
        while True:
            print("Temperatura:"+str(Temp_Ventilador))
            Temp_Ventilador=dth2.Temp()
            ventilador.fans(Temp_Ventilador)
            time.sleep(Hilos_FP.tempo_CV)
#**end             
